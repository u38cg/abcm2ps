                ...
                char *rfn) /* returned real file name */
{
  ...
  if (in_fname && in_fname != fn && (p = strrchr(in_fname, DIRSEP)) != NULL) {
    l = p - in_fname + 1;
...
static void treat_file(char *fn, char *ext) {
  ...
  if (!quiet)
    fprintf(strcmp(outfn, "-") == 0 ? stderr : stdout, "File %s\n", abc_fn);
  ...
  frontend((unsigned char *)file, file_type, abc_fn, 0);
  free(file);

  if (file_type == FE_PS) /* PostScript file */
    frontend((unsigned char *)"%%endps", FE_ABC, abc_fn, 0);
...
static void treat_abc_file(char *fn) {
  ...
  while (l > 0) {
    ...
    frontend((unsigned char *)file_tmp, FE_ABC, abc_fn, linenum);
    free(file_tmp);
...
static void set_opt(char *w, char *v) {
  ...
  if (strcmp(w, "abcm2ps") == 0)
    prefix = *v;
  frontend((unsigned char *)tex_buf, FE_ABC, "cmd_line", 0);
}
  ...
  while (--argc > 0) {
    ...
    if (c == '+') { /* switch off flags with '+' */
      while (*++p != '\0') {
        ...
        case '1':
          ...
          break;
        default:
          error(1, NULL, "++++ Cannot switch off flag: +%c", *p);
          break;
        }
      }
      continue;
    ...
    if (c == '-') {      /* interpret a flag with '-' */
      ...
      while ((c = *++p) != '\0') {
        ...
        case 'N':
          if (p[1] == '\0' && (argc <= 1 || !isdigit((unsigned)argv[1][0]))) {
        ...
        case 'w':
          ...
          if (*aaa == '\0') {
            aaa = *++argv;
            if (--argc <= 0 || (*aaa == '-' && c != 'O')) {
              error(1, NULL, "Missing parameter after '-%c' - aborting", c);
              return EXIT_FAILURE;
            }
          } else {
            p += strlen(p) - 1; /* stop */
          ...
          if (strchr("BbfjkNs", c)) { /* check num args */
            for (j = 0; j < strlen(aaa); j++) {
              if (!strchr("0123456789.", aaa[j])) {
                if (aaa[j] == 'b' && aaa[j + 1] == '\0' && c == 'j')
                  break;
                error(1, NULL, "Invalid parameter <%s> for flag -%c", aaa, c);
                return EXIT_FAILURE;
              }
            }
          }
          ...
          case 'N':
            ...
            if ((unsigned)pagenumbers > 4) {
              error(1, NULL, "'-N' value %s - changed to 2", aaa);
              pagenumbers = 2;
            }
            break;
          case 'O':
          ...
          break;
        default:
          error(1, NULL, "Unknown flag: -%c ignored", c);
          break;
        }
      }
      continue;
...
void *getarena(int len) {
  ...
  if (len > a_p->r) {
    if (len > MAXAREANASZ) {
      error(1, NULL, "getarena - data too wide %d - aborting", len);
      exit(EXIT_FAILURE);
    }
