...
static char char_tb[256] = {
    ...
    0,           0,          0,
    0,           0, /* 18 - 1f */
    CHAR_SPAC,   CHAR_DECOS, CHAR_GCHORD,
    CHAR_BAD, /* (sp) ! " # */
    CHAR_BAD,    CHAR_BAD,   CHAR_VOV,
    CHAR_BAD, /* $ % & ' */
    CHAR_OPAR,   CHAR_CPAR,  CHAR_BAD,
    CHAR_DECOS, /* ( ) * + */
    CHAR_BAD,    CHAR_MINUS, CHAR_DECO,
    CHAR_SLASH, /* , - . / */
    CHAR_BAD,    CHAR_BAD,   CHAR_BAD,
    CHAR_BAD, /* 0 1 2 3 */
    CHAR_BAD,    CHAR_BAD,   CHAR_BAD,
    CHAR_BAD, /* 4 5 6 7 */
    CHAR_BAD,    CHAR_BAD,   CHAR_BAR,
    CHAR_BAD, /* 8 9 : ; */
    CHAR_BRHY,   CHAR_NOTE,  CHAR_BRHY,
    CHAR_BAD, /* < = > ? */
    CHAR_BAD,    CHAR_NOTE,  CHAR_NOTE,
    CHAR_NOTE, /* @ A B C */
    CHAR_NOTE,   CHAR_NOTE,  CHAR_NOTE,
    CHAR_NOTE, /* D E F G */
    CHAR_DECO,   CHAR_DECO,  CHAR_DECO,
    CHAR_DECO, /* H I J K */
    CHAR_DECO,   CHAR_DECO,  CHAR_DECO,
    CHAR_DECO, /* L M N O */
    CHAR_DECO,   CHAR_DECO,  CHAR_DECO,
    ...
    CHAR_DECO,   CHAR_DECO,  CHAR_DECO,
    CHAR_DECO, /* T U V W */
    CHAR_NOTE,   CHAR_DECO,  CHAR_NOTE,
    CHAR_OBRA, /* X Y Z [ */
    CHAR_BSLASH, CHAR_BAR,   CHAR_NOTE,
    CHAR_NOTE, /* \ ] ^ _ */
    CHAR_IGN,    CHAR_NOTE,  CHAR_NOTE,
    CHAR_NOTE, /* ` a b c */
    CHAR_NOTE,   CHAR_NOTE,  CHAR_NOTE,
    CHAR_NOTE, /* d e f g */
    CHAR_DECO,   CHAR_DECO,  CHAR_DECO,
...
static char *parse_extra(char *p, char **p_name, char **p_middle,
                         char **p_stlines, char **p_scale, char **p_octave,
                         char **p_cue, char **p_map) {
  for (;;) {
    if (strncmp(p, "clef=", 5) == 0 || strncmp(p, "bass", 4) == 0 ||
        strncmp(p, "treble", 6) == 0 || strncmp(p, "alto", 4) == 0 ||
        strncmp(p, "tenor", 5) == 0 || strncmp(p, "perc", 4) == 0) {
      if (*p_name)
...
static char *get_deco(char *p, unsigned char *p_dc) {
  ...
  *p_dc = 0;
  ...
  if (*p == sep)
    p++;
  for (i = 1, t = &parse.deco_tb[1]; *t && i < DC_NAME_SZ; i++, t++) {
...
static void parse_clef(struct SYMBOL *s, char *name, char *middle) {
  ...
  if (name && clef < 0) {
    if (!strncmp(name, "bass", 4)) {
      ...
      clef = TREBLE;
      name += 6;
    } else if (!strncmp(name, "alto", 4) || !strncmp(name, "tenor", 5)) {
      clef = ALTO;
...
static void parse_key(char *p, struct SYMBOL *s) {
  ...
  parse_extra(p, &clef_name, &clef_middle, &clef_stlines, &clef_scale,
              &p_octave, &p_cue, &p_map);
...
static char *parse_voice(char *p, struct SYMBOL *s) {
  ...
  static struct kw_s {
    ...
    short len;
    short index;
  } kw_tb[] = {{"name=", 5, 0},    {"nm=", 3, 0},
               {"subname=", 8, 1}, {"sname=", 6, 1},
               {"snm=", 4, 1},     {"merge", 5, 2},
               {"up", 2, 3},       {"down", 4, 4},
               {"stem=", 5, 5},    {"gstem=", 6, 6},
               {"auto", 4, 7},     {"dyn=", 4, 8},
               {"lyrics=", 7, 9},  {"scale=", 6, 10},
               {"gchord=", 7, 11}, {0}};
  struct kw_s *kw;
  ...
  for (;;) {
    ...
    if (*p == '\0')
      break;
    p = parse_extra(p, &clef_name, &clef_middle, &clef_stlines, &clef_scale,
                    &p_octave, &p_cue, &p_map);
    if (*p == '\0')
      break;
...
char *parse_acc_pit(char *p, int *pit, int *acc) {
  ...
  if (*acc != 0 &&
      (isdigit((unsigned char)*p) || (*p == '/' && microscale == 0))) {
    int n, d;
    char *q;
...
static char *parse_deco(char *p, struct decos *deco,
                        int m) /* note index / -1 */
{
  int n;
  unsigned char t;
      ...
      if (strncasecmp(p, "linebreak ", 10) == 0) {
        for (i = 0; i < sizeof char_tb; i++) {
          if (char_tb[i] == CHAR_LINEBREAK)
            char_tb[i] = i != '!' ? CHAR_BAD : CHAR_DECOS;
        }
        p += 10;
        for (;;) {
          while (isspace((unsigned char)*p))
...
/*fixme:OK for [I:staff n], ?? for other headers*/
    ...
    default:
    bad_char:
      syntax((flags & ABC_F_GRACE) ? "Bad character in grace note sequence"
                                   : "Bad character",
             p - 1);
      break;
    }
  ...
  for (;;) {
    if (chord) {
      ...
      if (*p == '(') {
        ...
        s->u.note.notes[m].sl1 = (s->u.note.notes[m].sl1 << 3) + n;
      }
    }
    ...
    p = parse_len(p,
                  (flags & ABC_F_GRACE) ? BASE_LEN / 8 : // for grace note alone
                      ulen,
                  &len);
...
do_brhythm:
  if (curvoice->last_note && curvoice->last_note->u.note.brhythm != 0) {
    broken_rhythm(curvoice->last_note, curvoice->last_note->u.note.brhythm);
    broken_rhythm(s, -curvoice->last_note->u.note.brhythm);
  }
add_deco:
  if (dc.n > 0) {
    memcpy(s->abc_type != ABC_T_MREST ? &s->u.note.dc : &s->u.bar.dc, &dc,
           sizeof dc);
    dc.n = 0;
  }
