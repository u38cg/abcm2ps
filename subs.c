...
void error(int sev, /* 0: warning, 1: error */
           struct SYMBOL *s, char *fmt, ...) {
  va_list args;

  if (s) {
    if (s->fn)
      fprintf(stderr, "%s:%d:%d: ", s->fn, s->linenum, s->colnum);
    s->flags |= ABC_F_ERROR;
  }
...
float tex_str(char *s) {
  ...
  while (1) {
    ...
    case '\\':
      ...
      break;
    case '$':
      if (isdigit((unsigned char)*s) && (unsigned)(*s - '0') < FONT_UMAX) {
        i = *s - '0';
        if (i == 0)
          i = defft;
        swfac = cfmt.font_tb[i].swfac;
...
void pg_reset_font(void) { out_pg_ft = -1; }

static void desc_font(int fnum) {
  int i;
  char font_name[128], *p, *q;
  static char *bad_tb[] = {"Times-Roman", "Times", "Helvetica", "Courier"};
  static char *good_tb[] = {"serif", "serif", "sans-serif", "monospace"};
    ...
    for (i = 0; i < glyphs->num_glyphs; i++) {
      ...
      if (ret != 0) {
        ...
        if (FT_Get_Postscript_Name(face) != fontname) {
          ...
          if (glypharray)
            a2b("]glypharray");
          a2b("\n/%s %.1f selectfont[", fontname, (float)wi / PG_SCALE);
          glypharray = 1;
        }
        FT_Get_Glyph_Name(face, c, tmp, sizeof tmp);
        a2b("/%s", tmp);
      } else {
...
static void str_font_change(int start, int end) {
  struct FONTSPEC *f;
  int fnum;
...
static void str_set_font(char *p) {
  ...
  while (*p != '\0') {
    switch (*p) {
    case '$':
      if (isdigit((unsigned char)p[1]) && (unsigned)(p[1] - '0') < FONT_UMAX) {
        if (p > q)
    ...
    for (runs_list = line->runs; runs_list; runs_list = runs_list->next) {
      ...
      for (i = 0; i < glyphs->num_glyphs; i++) {
        ...
        if (set_move) {
          ...
          a2b("\n");
          a2b("%.2f %.2f M ", (float)x / PANGO_SCALE, -y);
        }
        ...
        if (ret != 0) {
          ...
          FT_Get_Glyph_Name((FT_FaceRec *)face, g, tmp, sizeof tmp);
          if (job == T_JUSTIFY && strcmp(tmp, "space") == 0) {
            set_move = 1;
            continue;
          }
          if (!glypharray) {
...
static int is_latin(unsigned char *p) {
  while (*p != '\0') {
    if (*p >= 0xc6) {
      if (*p == 0xe2) {
        if (p[1] != 0x99 || p[2] < 0xad || p[2] > 0xaf)
          return 0;
        p += 2;
      } else if (*p == 0xf0) {
        if (p[1] != 0x9d || p[2] != 0x84 || p[3] < 0xaa || p[3] > 0xab)
          return 0;
      } else {
        return 0;
      }
...
void str_font(int ft) { curft = defft = ft; }
...
static char *strop_tb[] = {
    /* index = action (A_xxxx) * 2 */
    "show",   "arrayshow", // left
    ...
    case '$':
      if (isdigit((unsigned char)p[1]) && (unsigned)(p[1] - '0') < FONT_UMAX) {
        ...
        if (curft != p[1] - '0') {
          ...
          if (curft == 0)
            curft = defft;
          use_glyph = !svg && epsf <= 1 && get_font_encoding(curft) == 0;
        }
        p += 2;
        q = p;
        continue;
...
static void put_inf2r(struct SYMBOL *s1, struct SYMBOL *s2, int action) {
  char buf[256], *p, *q;
...
static int put_wline(char *p, float x, int right) {
  char *q, *r, sep;
  ...
  r = NULL;
  q = p;
  if (isdigit((unsigned char)*p) || (*p != '\0' && p[1] == '.')) {
    while (*p != '\0') {
      p++;
      if (*p == ' ' || p[-1] == ':' || p[-1] == '.')
        break;
...
void put_history(void) {
  ...
  for (s = info['I' - 'A']; s; s = s->next) {
    u = s->text[0] - 'A';
    if (!(cfmt.fields[0] & (1 << u)) || (s2 = info[u]) == NULL)
      continue;
    if (!font) {
...
char *trim_title(char *p, struct SYMBOL *title) {
  ...
  if (title != info['T' - 'A'] || !(cfmt.fields[0] & (1 << ('X' - 'A'))))
    title = NULL;
  if (!q && !title && !cfmt.titlecaps)
...
static void write_headform(float lwidth) {
  ...
  for (;;) {
    ...
    if ((unsigned)i < 26) {
      ...
      if (j > 0 && fmt[j - 1] < 125 && j < sizeof fmt - 4) {
...
void write_heading(void) {
  ...
  rhythm = ((first_voice->key.instr == K_HP || first_voice->key.instr == K_Hp ||
             pipeformat) &&
            !cfmt.infoline && (cfmt.fields[0] & (1 << ('R' - 'A'))))
               ? info['R' - 'A']
               : NULL;
  if (rhythm) {
  ...
  if (composer || origin || author || cfmt.infoline) {
    ...
    if (composer || origin) {
      ...
      for (;;) {
        bskip(cfmt.font_tb[COMPOSERFONT].size);
        a2b("%.1f 0 M ", xcomp);
        put_inf2r(s, (!s || !s->next) ? origin : NULL, align);
        if (!s)
          break;
        if ((s = s->next) == NULL)
          break;
  ...
  /* parts */
  if (info['P' - 'A'] && (cfmt.fields[0] & (1 << ('P' - 'A')))) {
  ...
  for (t = user_ps; t; t = t->next) {
    ...
    case '\001': { /* PS file */
      ...
      if ((f = fopen(p + 1, "r")) == NULL) {
        error(1, NULL, "Cannot open PS file '%s'", &t->text[1]);
      } else {
