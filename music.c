...
float space_tb[NFLAGS_SZ] = {7,    10, 14.15, 20, 28.3, 40, /* crotchet */
                             56.6, 80, 113,   150};
...
static char rest_sp[NFLAGS_SZ][2] = {{18, 18}, {12, 18}, {12, 12}, {8, 12},
                                     {6, 8},   {10, 10}, /* crotchet */
                                     {6, 4},   {10, 0},  {10, 4},  {10, 10}};
...
static float set_heads(struct SYMBOL *s) {
  ...
  char *p, *q, *r;
  float w, wmax;
  static float dx_tb[4] = {7, 8, 10, 13.3};
...
static void set_acc_shft(void) {
  ...
  while (s) {
    ...
    for (s2 = s; s2; s2 = s2->ts_next) {
      if (s2->time != t || s2->abc_type != ABC_T_NOTE || s2->staff != staff)
        break;
      if (acc)
        continue;
      for (i = 0; i <= s2->nhd; i++) {
...
void unlksym(struct SYMBOL *s) {
  ...
  if (s->ts_next) {
    ...
    if ((s->sflags & S_SEQST) && !(s->ts_next->sflags & S_SEQST)) {
...
static int may_combine(struct SYMBOL *s) {
  ...
  if (s->combine == 0 && s->abc_type != ABC_T_REST)
    return 0;
  s2 = s->ts_next;
  if (!s2 || s2->type != NOTEREST)
    return 0;
  if (s2->voice == s->voice || s2->staff != s->staff || s2->time != s->time ||
      s2->dur != s->dur)
    return 0;
  if (s->combine <= 0 && s2->abc_type != s->abc_type)
    return 0;
  ...
  if (s->abc_type == ABC_T_REST) {
    if (s2->abc_type == ABC_T_REST && (s->flags & ABC_F_INVIS) &&
        !(s2->flags & ABC_F_INVIS))
      return 0;
    return 1;
  }
  if (s2->ly || (s2->sflags & (S_SL1 | S_SL2)) || s2->u.note.slur_st != 0 ||
  ...
  if (s->nhd + nhd2 + 1 >= MAXHD)
    return 0;
  if (s->combine <= 1 && s->pits[0] <= s2->pits[nhd2] + 1)
    return 0;
  return 1;
}
...
again:
  ...
  if (s->abc_type == ABC_T_REST) {
    if ((s->flags & ABC_F_INVIS) && !(s2->flags & ABC_F_INVIS))
      s->flags &= ~ABC_F_INVIS;
    goto delsym2;
  }
  ...
  /* combine the voices */
  nhd2 = s2->nhd + 1;
  memcpy(&s->u.note.notes[nhd + 1], s2->u.note.notes,
         sizeof s->u.note.notes[0] * nhd2);
  memcpy(&s->pits[nhd + 1], s2->pits, sizeof s->pits[0] * (nhd2 + 1));
  ...
  if (s->combine >= 3) { // remove unison heads
    for (m = nhd; m > 0; m--) {
      if (s->u.note.notes[m].pit == s->u.note.notes[m - 1].pit &&
        ...
        i = nhd - m;
        if (i > 0) {
          memmove(&s->u.note.notes[m], &s->u.note.notes[m + 1],
                  sizeof s->u.note.notes[0] * i);
          memmove(&s->pits[m], &s->pits[m + 1], sizeof s->pits[0] * i);
        }
        s->nhd = --nhd;
      }
    }
...
delsym2:
  ...
  if (s2->u.note.dc.n > 0) { // update the added decorations
    ...
    n = s->u.note.dc.n;
    memcpy(&s->u.note.dc.tm[n], s2->u.note.dc.tm,
...
static void combine_voices(void) {
  ...
  for (s = tsfirst; s->ts_next; s = s->ts_next) {
    ...
    if (s->sflags & S_IN_TUPLET) {
      ...
      r = 0;
      for (; g; g = g->next) {
        if (g->type == TUPLET && g->u.tuplet.r_plet > r)
          r = g->u.tuplet.r_plet;
      }
      if (r == 0)
        continue;
...
static struct SYMBOL *insert_clef(struct SYMBOL *s, int clef_type,
                                  int clef_line) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    for (s = p_voice->sym; s; s = s->next) {
      ...
      for (s1 = s->ts_prev; s1; s1 = s1->ts_prev) {
        if (s1->staff != staff || s1->voice == s->voice)
          break;
#if 1
        /*fixme:test again*/
      ...
      for (s1 = s->ts_next; s1; s1 = s1->ts_next) {
        if (s1->staff != staff + 1 || s1->voice == s->voice)
          break;
#if 1
        /*fixme:test again*/
        if (s1->abc_type == ABC_T_NOTE)
...
static float set_graceoffs(struct SYMBOL *s) {
  ...
  xx += gspleft + gspright;
  next = s->next;
  if (next && next->abc_type == ABC_T_NOTE) { /* if before a note */
    if (g->y >= 3 * (next->pits[next->nhd] - 18))
      xx -= 1; /* above, a bit closer */
    else if ((g->sflags & S_BEAM_ST) && g->y < 3 * (next->pits[0] - 18) - 7)
...
static float gchord_width(struct SYMBOL *s, float wlnote, float wlw) {
  struct SYMBOL *s2;
  struct gch *gch;
  int ix;
...
static float ly_width(struct SYMBOL *s, float wlw) {
  ...
  for (i = 0; i < MAXLY; i++) {
    ...
    if (s->type == GRACE) { // %%graceword
      shift = s->wl;
    } else if ((isdigit((unsigned char)*p) && strlen(p) > 2) || p[1] == ':' ||
               *p == '(' || *p == ')') {
      float sz;
      ...
      sz *= swfac;
      shift = (w - sz + 2 * cwid(' ') * swfac) * VOCPRE;
      if (shift > 20)
        shift = 20;
      shift += sz;
    ...
    for (m = 0; m <= s->nhd; m++) {
      ...
      if (s->u.note.notes[m].acc) {
        AT_LEAST(wlnote, s->u.note.notes[m].shac +
                             ((s->u.note.notes[m].acc & 0xf8) ? 6.5 : 4.5));
      }
    }
    if (s2) {
      switch (s2->type) {
...
static float set_space(struct SYMBOL *s) {
  ...
  if (s->ts_prev->type == MREST)
    return s->ts_prev->wr + 16 + 3; // (bar wl=5 wr=8)
  ...
  if (s->abc_type == ABC_T_NOTE && s->nflags >= -1 && s->stem > 0) {
    stemdir = 1;
  ...
  while (s != last_s) {
    ...
    do {
      ...
      for (s3 = s->ts_prev; s3; s3 = s3->ts_prev) {
        if (new_val < s3->wr && s3->type == NOTEREST && s2->type == NOTEREST)
          new_val = s3->wr;
        if (s3->staff == s2->staff &&
            (!(s3->flags & ABC_F_INVIS) || s3->voice == s2->voice) &&
            new_val < s3->wr + wl) {
          switch (s3->type) {
          case NOTEREST:
            if (s2->type == NOTEREST) {
                       ...
                       struct SYMBOL *s) /* first note */
{
  ...
  if ((n = g->doty) < 0) { /* number of notes / measures */
    ...
    if (!s2 || !s2->next) { /* should have some symbol */
    ...
    for (j = g->nohdi1; --j >= 0;) {
      ...
      for (s = s2; s; s = s->ts_next) {
        if (s->staff == staff && s->voice == voice && s->dur)
          break;
      }
    }
    goto delrep; /* done */
...
setnl:
  if (cfmt.custos && !first_voice->next) {
    ...
    default: /* add an extra symbol at eol */
      ...
      // {	/* auto break */
      //				for (s2 = s->ts_next; ; s2 =
      //s2->ts_next) { 					if (s2->x != 0) { 						extra->x = s2->x - 1; 						break;
      //					}
      //				}
      //			}
      break;
                                ...
                                float indent)         /* for start of tune */
{
  ...
  for (;;) {
    ...
    beam = s2->type == NOTEREST && (s2->sflags & (S_BEAM_ST | S_BEAM_END)) == 0
               ? 1
               : 0;
    ...
    for (; s != last; s = s->ts_next) {
      ...
      if ((s->sflags & (S_BEAM_ST | S_BEAM_END)) == S_BEAM_END)
        beam--;
      x = s->x;
      if (x < xmin)
        continue;
...
/*fixme: may change in the tune*/
  ...
  for (; s; s = s->ts_next) {
    ...
    if (xmin > lwidth) {
      if (cfmt.linewarn)
        error(0, s, "Line overfull (%.0fpt of %.0fpt)", xmin, lwidth);
...
static void set_yval(struct SYMBOL *s) {
  ...
  switch (s->type) {
  case CLEF:
    if ((s->sflags & S_SECOND) || (s->flags & ABC_F_INVIS)) {
...
static int set_auto_clef(int staff, struct SYMBOL *s_start,
                         int clef_type_start) {
  struct SYMBOL *s;
  ...
  for (s = s_start; s != s_last; s = s->ts_next) {
    ...
    if (clef_type == TREBLE) {
      ...
      s2 = s->ts_prev;
      if (s2 && s2->time == time && s2->staff == staff &&
          s2->abc_type == ABC_T_NOTE && s2->pits[0] >= 19) /* F */
        continue;
      s2 = s->ts_next;
      if (s2 && s2->staff == staff && s2->time == time &&
          s2->abc_type == ABC_T_NOTE && s2->pits[0] >= 19) /* F */
        continue;
    } else {
      if (s->pits[0] < 12            /* F, */
      ...
      s2 = s->ts_prev;
      if (s2 && s2->time == time && s2->staff == staff &&
          s2->abc_type == ABC_T_NOTE && s2->pits[0] <= 13) /* G, */
        continue;
      s2 = s->ts_next;
      if (s2 && s2->staff == staff && s2->time == time &&
          s2->abc_type == ABC_T_NOTE && s2->pits[0] <= 13) /* G, */
    ...
    if (!s_last_chg) {
      clef_type = clef_type_start = clef_type == TREBLE ? BASS : TREBLE;
      s_last_chg = s;
      continue;
    }
    ...
    for (s2 = s->ts_prev; s2 != s_last_chg; s2 = s2->ts_prev) {
      if (s2->staff != staff)
        continue;
      if (s2->type == BAR && s2->voice == s->voice) {
        s3 = s2;
        break;
      }
      if (s2->abc_type != ABC_T_NOTE)
      ...
      if ((s2->sflags & S_BEAM_ST) && !voice_tb[s2->voice].second)
        s3 = s2;
    }
...
static void set_clefs(void) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    s = p_voice->s_clef;
    if (!sy->voice[voice].second && !(s->sflags & S_CLEF_AUTO))
      staff_clef[staff].autoclef = 0;
  }
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    voice = p_voice - voice_tb;
    if (sy->voice[voice].range < 0 || sy->voice[voice].second) // main voices
      continue;
    staff = sy->voice[voice].staff;
    s = p_voice->s_clef;
    if (staff_clef[staff].autoclef) {
      s->u.clef.type = set_auto_clef(staff, tsfirst, s->u.clef.type);
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    if (s->sflags & S_NEW_SY) {
      ...
      for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
        voice = p_voice - voice_tb;
        if (sy->voice[voice].range < 0 || sy->voice[voice].second)
          continue;
        staff = sy->voice[voice].staff;
        s2 = p_voice->s_clef;
        if (s2->sflags & S_CLEF_AUTO) {
        ...
        if (!staff_clef[staff].clef) { // new staff
          if (s2->sflags & S_CLEF_AUTO) {
            if (s2->u.clef.type != AUTOCLEF) {
              p_voice->s_clef = (struct SYMBOL *)getarena(sizeof *s);
              memcpy(p_voice->s_clef, s2, sizeof *p_voice->s_clef);
            }
            p_voice->s_clef->u.clef.type = new_type;
            p_voice->s_clef->u.clef.line = new_line;
          }
          staff_tb[staff].s_clef = staff_clef[staff].clef = p_voice->s_clef;
          continue;
        }
        // old staff
    ...
    if (s->u.clef.type == AUTOCLEF) {
      s->u.clef.type = set_auto_clef(s->staff, s->ts_next,
...
static void set_pitch(struct SYMBOL *last_s) {
  ...
  static const signed char delta_tb[4] = {0 - 2 * 2, 6 - 3 * 2, 12 - 4 * 2,
                                          0 - 3 * 2};

  for (staff = 0; staff <= nstaff; staff++) {
    s = staff_tb[staff].s_clef;
    staff_delta[staff] =
        delta_tb[s->u.clef.type] + s->u.clef.line * 2 + s->u.clef.transpose;
  }
  ...
  for (s = tsfirst; s != last_s; s = s->ts_next) {
    ...
    case GRACE:
      for (g = s->extra; g; g = g->next) {
        ...
        if (delta != 0 && voice_tb[s->voice].key.instr != K_DRUM) {
          for (m = g->nhd; m >= 0; m--)
            g->pits[m] += delta;
        }
      ...
      break;
    case KEYSIG:
      s->u.key.clef_delta = staff_delta[staff]; /* keep the current clef */
                                                //			s->ymx = 24 + 10;
                                                //			s->ymn = -2;
                                                //			break;
                                                /* fall thru */
    ...
    case MREST:
      ...
      break;
    case NOTEREST:
      if (s->abc_type != ABC_T_NOTE && !first_voice->next) {
...
static void set_stem_dir(void) {
  ...
  while (s) {
    ...
    for (u = s; u; u = u->ts_next) {
      ...
      if (u->type != NOTEREST || (u->flags & ABC_F_INVIS))
        continue;
      staff = u->staff;
#if 1
      /*fixme:test*/
      ...
      if (i < 0) {
        ...
        for (i = 0; i < stb[staff].nvoice; i++) {
          if (rvoice < stb[staff].st[i].voice) {
            memmove(&stb[staff].st[i + 1], &stb[staff].st[i],
                    sizeof stb[staff].st[i] * (stb[staff].nvoice - i));
            stb[staff].st[i].ymx = 0;
            stb[staff].st[i].ymn = 24;
            break;
          }
      ...
      if (u->sflags & S_XSTEM) {
        if (!u->ts_prev || u->ts_prev->staff != staff - 1 ||
    ...
    for (; s != u; s = s->ts_next) {
      ...
      if (i == stb[staff].nvoice) {
        ...
        if (i != 0 && i + 1 == stb[staff].nvoice) {
          if (stb[staff].st[i].ymn - cfmt.stemheight > stb[staff].st[i + 1].ymx)
            s->multi = -1;

          /* special case for unison */
          if (s->ts_prev && s->ts_prev->time == s->time &&
              s->ts_prev->staff == s->staff &&
              s->pits[s->nhd] == s->ts_prev->pits[0] &&
              (s->sflags & (S_BEAM_ST | S_BEAM_END)) ==
                  (S_BEAM_ST | S_BEAM_END) &&
              ((t = s->ts_next) == NULL || t->staff != s->staff ||
               t->time != s->time))
            s->multi = -1;
        }
      }
...
static void set_rest_offset(void) {
  struct SYSTEM *sy;
  struct SYMBOL *s, *s2;
  int nvoice, voice, end_time, not_alone, ymax, ymin, shift, dots;
  float dx;
  struct {
    struct SYMBOL *s;
    int staff;
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    for (voice = 0, v = vtb; voice <= nvoice; voice++, v++) {
      s2 = v->s;
      if (!s2 || v->staff != s->staff || voice == s->voice)
        continue;
    ...
    for (s2 = s->ts_next; s2; s2 = s2->ts_next) {
      if (s2->time >= end_time)
        break;
      if (s2->staff != s->staff || s2->type != NOTEREST ||
          (s2->flags & ABC_F_INVIS))
        continue;
      not_alone++;
...
static struct SYMBOL *sym_new(int type, struct VOICE_S *p_voice,
...
static void init_music_line(void) {
  ...
  while (last_s && last_s->type == CLEF) { /* move the starting clefs */
    ...
    if (cursys->voice[voice].range >= 0 && !cursys->voice[voice].second) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    s = (struct SYMBOL *)getarena(sizeof *s);
    memset(s, 0, sizeof *s);
    memcpy(&s->u.clef, &staff_tb[staff].s_clef->u.clef, sizeof s->u.clef);
    s->type = CLEF;
    s->voice = voice;
    s->staff = staff;
    ...
    //			s->sflags |= S_SECOND;
    if (staff_tb[staff].s_clef->u.clef.invis || cursys->staff[staff].empty)
      s->flags |= ABC_F_INVIS;
    //		set_yval(s);
  }
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    if (cursys->voice[voice].range < 0 || cursys->voice[voice].second ||
      ...
      continue;
    if (last_s && last_s->voice == voice && last_s->type == KEYSIG) {
  ...
  if (insert_meter & 1) {
    for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
      ...
      if (cursys->voice[voice].range < 0 || cursys->voice[voice].second ||
        ...
        continue;
      if (last_s && last_s->voice == voice && last_s->type == TIMESIG) {
        p_voice->last_sym = last_s;
        last_s = last_s->ts_next;
        continue;
      }
      s = sym_new(TIMESIG, p_voice, last_s);
      memcpy(&s->u.meter, &p_voice->meter, sizeof s->u.meter);
      //			set_yval(s);
    }
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    if (last_s && last_s->voice == voice && last_s->type == BAR) {
      p_voice->last_sym = last_s;
      last_s = last_s->ts_next;
      continue;
    }
...
static void set_words(struct VOICE_S *p_voice) {
  ...
  for (s = p_voice->sym; s; s = s->next) {
    ...
    case BAR:
      ...
      if (!s->next && s->prev && s->prev->abc_type == ABC_T_NOTE &&
          s->prev->dur >= BREVE)
    ...
    case NOTEREST:
      ...
      if (s->flags & ABC_F_SPACE)
        beam_start = 1;
      if (beam_start || s->nflags - s->aux <= 0) {
        if (lastnote) {
...
static float set_indent(void) {
  ...
  if (maxw != 0) {
    ...
    for (staff = 0; staff <= cursys->nstaff; staff++) {
      if (cursys->staff[staff].flags & (OPEN_BRACE2 | OPEN_BRACKET2)) {
        w = 20;
        break;
      }
      if ((cursys->staff[staff].flags & (OPEN_BRACE | OPEN_BRACKET)) && w == 0)
        w = 10;
    }
  ...
  for (s = sym; s; s = s->next) {
    ...
    if (s->stem == 0                    /* if not explicitly set */
      ...
      if (beam) {
        ...
        int pu = s->pits[s->nhd], pd = s->pits[0];
...
static int same_head(struct SYMBOL *s1, struct SYMBOL *s2) {
  ...
  if (s1->dots != s2->dots) {
    //		if ((s1->sflags & (S_SHIFTUNISON_1 | S_SHIFTUNISON_2))
    if ((s1->sflags & S_SHIFTUNISON_1) || s1->dots * s2->dots != 0)
      return 0;
  }
...
static float w_note[] = {3.5, 3.7, 5, 7};

/* handle unison with different accidentals */
static void unison_acc(struct SYMBOL *s1, struct SYMBOL *s2, int i1, int i2) {
  int m;
  float d;
...
static void set_right(struct SYMBOL *s, float *right) {
  ...
  /* stem and flags */
  flags = s->nflags > 0 &&
          (s->sflags & (S_BEAM_ST | S_BEAM_END)) == (S_BEAM_ST | S_BEAM_END);
  w = w_base = w_note[s->head];
  if (s->nflags > -2) {
    if (s->stem < 0) {
      w = -w;
...
static void set_overlap(void) {
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    if ((s->sflags & S_XSTEM) && s->ts_prev->stem < 0) {
      s2 = s->ts_prev;
      for (m = 0; m <= s2->nhd; m++) {
    ...
    for (;;) {
      ...
      if (s2->abc_type == ABC_T_NOTE && !(s2->flags & ABC_F_INVIS) &&
          s2->staff == s->staff)
        break;
    }
    if (!s2)
    ...
    s3 = s1->ts_prev;
    if (s3 && s3->time == s1->time && s3->staff == s1->staff &&
        s3->abc_type == ABC_T_NOTE && !(s3->flags & ABC_F_INVIS)) {
      set_right(s3, right3);
      for (i = 0; i < MAXPIT; i++) {
    ...
    if (d < -3) { // no clash if no dots clash
      if (!s1->dots || !s2->dots || s2->doty >= 0 || s1->stem > 0 ||
          s2->stem < 0 || s1->pits[s1->nhd] + 2 != s2->pits[0] ||
          (s2->pits[0] & 1))
        continue;
    }
    ...
    for (;;) {
      ...
      switch (dp) {
      case 0:
        if (s1->u.note.notes[i1].acc != s2->u.note.notes[i2].acc) {
          t = -1;
          break;
        }
        if (s2->u.note.notes[i2].acc)
          s2->u.note.notes[i2].acc = 0;
        if (s1->dots && s2->dots && (s1->pits[i1] & 1))
          t = 1;
        break;
      case -1:
        //				if (s1->dots && s2->dots)
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    if (s->flags & ABC_F_STEMLESS) {
      ...
      if (nflags >= 2)
        slen -= 1;
      if (s->pits[s->nhd] > 26 &&
          (nflags <= 0 || (s->sflags & (S_BEAM_ST | S_BEAM_END)) !=
                              (S_BEAM_ST | S_BEAM_END))) {
        slen -= 2;
        if (s->pits[s->nhd] > 28)
          slen -= 2;
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    case MREST:
      if (cfmt.staffnonote > 1) {
        ...
        if (s->abc_type == ABC_T_NOTE || cfmt.staffnonote != 0)
          empty_gl[s->staff] = empty[s->staff] = 0;
      }
      break;
    }
...
static void set_sym_glue(float width) {
  ...
  if (tsnext) {
    if (x - width >= 0) {
      ...
      if (beta_last > beta0) {
        if (cfmt.stretchstaff) {
          if (!cfmt.continueall && cfmt.linewarn) {
            error(0, s, "Line underfull (%.0fpt of %.0fpt)",
                  beta0 * xmax + (1 - beta0) * x, width);
          }
        } else {
          width = x;
          beta_last = 0;
  ...
  if (x >= width) {
    ...
    if (x == xmin) {
      ...
      if (alfa > 1) {
        error(1, s, "Line too much shrunk (%.0f/%0.fpt of %.0fpt)", xmin, x,
              width);
