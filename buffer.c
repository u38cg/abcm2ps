...
void open_fout(void) {
  ...
  if (i < 0) {
    ...
    if (fnm[i] == '=' && in_fname) {
      ...
      strcpy(&fnm[i + 1], svg || epsf > 1 ? "Out.xhtml" : OUTPUTFILE);
    }
#if 0
...
static void close_fout(void) {
  ...
  if (epsf || svg == 1)
    ...
    printf("Output written on %s (%d page%s, %d title%s, %ld bytes)\n", outfnam,
           nbpages, nbpages == 1 ? "" : "s", tunenum, tunenum == 1 ? "" : "s",
           m);
out1:
  fclose(fout);
out2:
...
static float headfooter(int header, float pwidth, float pheight) {
  ...
  for (;;) {
    ...
    if ((q = strchr(p, '\t')) != NULL) {
      if (q != p) {
        *q = '\0';
        output(fout, "%.1f %.1f M ", p_fmt->leftmargin, y);
        mbf = tmp;
        str_out(p, A_LEFT);
        a2b("\n");
        if (svg)
    ...
    /* center */
    if (q != p) {
      output(fout, "%.1f %.1f M ", pwidth * 0.5, y);
      mbf = tmp;
      str_out(p, A_CENTER);
      a2b("\n");
      if (svg)
    ...
    if (q) {
      p = q + 1;
      if (*p != '\0') {
        output(fout, "%.1f %.1f M ", pwidth - p_fmt->rightmargin, y);
        mbf = tmp;
        str_out(p, A_RIGHT);
        a2b("\n");
        if (svg)
