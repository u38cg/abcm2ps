...
static char *std_deco_tb[] = {"dot 0 stc 5 1 1",
                              "roll 3 cpu 7 6 6",
...
static void d_gliss(struct deco_elt *de2) {
  ...
  if (s1->dots)
    de1->x += 5 + s1->xmx;
  s2 = de2->s;
  de2->x -= 2 + s2->u.note.notes[0].shac ? (s2->u.note.notes[0].shac + 3)
                                         : hw_tb[s2->head];
}
...
static void d_trill(struct deco_elt *de) {
  ...
  if (s2->sflags & S_OTTAVA) {
    ...
    for (de2 = de->next; de2; de2 = de2->next) {
      if (de2->t == de->t && de2->s->time == s2->time &&
  ...
  s = de->start->s;
  x = s->x;
  if (s->abc_type == ABC_T_NOTE && s->u.note.dc.n > 1)
    x += 10;
  //	} else {			/* end without start */
  //		s = first_note;
  //		if (!s) {
...
static void d_upstaff(struct deco_elt *de) {
  ...
  if (strcmp(ps_name, "accent") == 0 || strcmp(ps_name, "cpu") == 0) {
    if (!up || (up < 0 && (s->multi < 0 || (s->multi == 0 && s->stem > 0)))) {
  ...
  } else if (strcmp(ps_name, "brth") == 0 || strcmp(ps_name, "lphr") == 0 ||
             strcmp(ps_name, "mphr") == 0 || strcmp(ps_name, "sphr") == 0) {
    yc = stafft + 1;
...
static int get_deco(char *name) {
  ...
  for (ideco = 1, dd = &deco_def_tb[1]; ideco < 128; ideco++, dd++) {
    if (!dd->name || strcmp(dd->name, name) == 0)
      return ideco;
  }
...
static unsigned char deco_build(char *name, char *text) {
  ...
  if (sscanf(text, "%15s %d %15s %d %d %d%n", name2, &c_func, ps_func, &h, &wl,
             &wr, &n) != 6) {
    error(1, NULL, "Invalid %%%%deco %s", text);
    return 128;
  }

  if ((unsigned)c_func > 10 && (c_func < 32 || c_func > 41)) {
  ...
  for (ps_x = 0; ps_x < sizeof ps_func_tb / sizeof ps_func_tb[0]; ps_x++) {
    if (ps_func_tb[ps_x] == 0 || strcmp(ps_func_tb[ps_x], ps_func) == 0)
      break;
  }
  ...
  if (strcmp(text, name) == 0) {
    ...
    for (strx = 1; strx < sizeof str_tb / sizeof str_tb[0]; strx++) {
      if (!str_tb[strx]) {
        if (*text == '"') {
          text++;
          l = strlen(text);
...
static void set_feathered_beam(struct SYMBOL *s1, int accel) {
  struct SYMBOL *s, *s2;
  int n, t, tt, d, b, i;
  float a;
  ...
  s2 = NULL;
  n = 1;
  for (s = (struct SYMBOL *)s1->abc_next; s; s = (struct SYMBOL *)s->abc_next) {
    if (s->dur != d || (s->flags & ABC_F_SPACE))
      break;
    s2 = s;
    n++;
  }
  ...
  t = 0;
  if (accel) { /* !beam-accel! */
    for (s = s1, i = n - 1; s != s2; s = (struct SYMBOL *)s->abc_next, i--) {
      d = (int)lroundf(a * i) + b;
      s->dur = d;
      t += d;
    }
  } else { /* !beam-rall! */
    for (s = s1, i = 0; s != s2; s = (struct SYMBOL *)s->abc_next, i++) {
      d = (int)lroundf(a * i) + b;
      s->dur = d;
      t += d;
    }
...
static unsigned char deco_define(char *name) {
  ...
  for (d = user_deco; d; d = d->next) {
    if (strncmp(d->text, name, l) == 0 && d->text[l] == ' ')
...
static unsigned char deco_intern(unsigned char ideco, struct SYMBOL *s) {
  char *name;
  ...
  for (i = dc->n; --i >= 0;) {
    ...
    case 1: // slide
      if (s->abc_type != ABC_T_NOTE && s->abc_type != ABC_T_REST) {
        error(1, s, "!%s! must be on a note or a rest", dd->name);
        break;
      }
      continue;
    case 8: // gliss: move to the upper note of the chord
      if (s->abc_type != ABC_T_NOTE) {
        error(1, s, "!%s! must be on a note", dd->name);
        break;
      }
      if (m < 0)
        dc->tm[i].m = s->nhd;
    ...
    case 9: // move the alternate head of the chord to the notes
      ...
      continue;
    default:
      if (strlen(dd->name) >= 4 && dd->name[0] == '8' && dd->name[1] == 'v' &&
          dd->name[4] == '\0') {
        if (dd->name[3] == '(') {
          if (dd->name[2] == 'a')
            curvoice->ottava = -7;
    ...
    case 33: /* beamon */
      ...
      break;
    case 34: /* trem1..trem4 */
      if (s->abc_type != ABC_T_NOTE || !prev || prev->abc_type != ABC_T_NOTE ||
          s->dur != prev->dur) {
        error(1, s, "!%s! must be on the last of a couple of notes", dd->name);
        break;
      }
      if (strlen(dd->name) != 5)
        n = 0;
    ...
    case 36: /* beambr1 / beambr2 */
      ...
      if (strlen(dd->name) != 7 || (dd->name[6] != '1' && dd->name[6] != '2')) {
        error(1, s, "bad definition of !%s!", dd->name);
        break;
      }
      s->sflags |= dd->name[6] == '1' ? S_BEAM_BR1 : S_BEAM_BR2;
      break;
    case 37: /* rbstop */
      s->sflags |= S_RBSTOP;
      break;
  ...
  for (de = deco_head; de; de = de->next) {
    ...
    if (de->m >= 0) {
      ...
      if (de->flags & DE_UP)
        ...
        ym = ymid[staff++];
      ym -= dd->h * 0.5;
      if (((de->flags & DE_UP) && y < ym) || (!(de->flags & DE_UP) && y > ym)) {
        //				if (s->staff > staff) {
        //					while (s->staff != staff)
        //						s = s->ts_prev;
        ...
        //						s = s->ts_next;
        //				}
        y2 = y_get(staff, !(de->flags & DE_UP), de->x, de->val) +
             staff_tb[staff].y;
        if (de->flags & DE_UP)
          y2 -= dd->h;
        if (((de->flags & DE_UP) && y2 > ym) ||
          ...
          y = ym;
          y_set(staff, de->flags & DE_UP, de->x, de->val,
                ((de->flags & DE_UP) ? y + dd->h : y) - staff_tb[staff].y);
        }
      }
    }
    ...
    set_defl(de->defl);
    if (de->flags & DE_VAL) {
      if (dd->func != 2 || voice_tb[s->voice].scale != 1)
        putx(de->val);
      else
        putf(de->val);
    }
...
static void deco_create(struct SYMBOL *s, struct decos *dc) {
  int k, m, posit;
  unsigned char ideco;
  struct deco_def_s *dd;
  struct deco_elt *de;
...
static void ll_deco(void) {
  ...
  for (de = deco_head;; de = de->next) {
    ...
    for (de2 = de->next; de2; de2 = de2->next)
      if (!de2->start && de2->t == t && de2->s->voice == voice)
        break;
    if (!de2) { /* no end, search in the staff */
      staff = de->s->staff;
      for (de2 = de->next; de2; de2 = de2->next)
        if (!de2->start && de2->t == t && de2->s->staff == staff)
          break;
    }
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    case GRACE:
      for (g = s->extra; g; g = g->next) {
        if (g->abc_type != ABC_T_NOTE || g->u.note.dc.n == 0)
          continue;
        dc = &g->u.note.dc;
        deco_create(g, dc);
      }
...
static void draw_repbra(struct VOICE_S *p_voice) {
  ...
  for (s = p_voice->sym->next; s; s = s->next) {
    if (s->type != BAR)
      continue;
    if (!(s->sflags & S_RBSTART) || (s->sflags & S_NOREPBRA))
      continue;
  ...
  for (; s; s = s->next) {
    ...
    if (s2->type != BAR) {
      ...
      if (s1->staff > 0 && !(cursys->staff[s1->staff - 1].flags & STOP_BAR))
        w = s2->wl;
    ...
    if (!s2->next /* 2nd ending at end of line */
        && !(s2->sflags & S_RBSTOP) && (p_voice->bar_start == 0)) {
    ...
    a2b("(%s)-%.1f %d ", p, cfmt.font_tb[REPEATFONT].size * 0.8 + 1, i);
    putx(w);
...
void draw_deco_staff(void) {
  ...
  if (first_gchord) {
    ...
    for (s = first_gchord; s; s = s->ts_next) {
      ...
      draw_gchord(s, minmax[s->staff].ymin, minmax[s->staff].ymax);
    }
  }
  ...
  for (de = deco_head; de; de = de->next) {
    ...
    dd = &deco_def_tb[de->t];
    if (dd->ps_func < 0 || !(f_staff & (1 << dd->func)))
      continue;
    if (cfmt.dynalign) {
      if (de->flags & DE_UP)
...
float tempo_width(struct SYMBOL *s) {
  ...
  if (s->u.tempo.beats[0] != 0) {
    if (s->u.tempo.circa)
      w += tex_str("ca. ");
    i = 1;
    while (i < sizeof s->u.tempo.beats / sizeof s->u.tempo.beats[0] &&
           s->u.tempo.beats[i] != 0) {
      w += 10;
      i++;
    }
    w += 6 + cwid(' ') * cfmt.font_tb[TEMPOFONT].swfac * 6 + 10 + 10;
  }
  if (s->u.tempo.str2)
...
void write_tempo(struct SYMBOL *s, int beat, float sc) {
  ...
  if (s->u.tempo.beats[0] != 0) {
    ...
    for (i = 0; i < sizeof s->u.tempo.beats / sizeof s->u.tempo.beats[0] &&
                s->u.tempo.beats[i] != 0;
         i++) {
...
float draw_partempo(int staff, float top) {
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    if (cfmt.partsbox)
      a2b(" %.1f %.1f %.1f boxend boxdraw", s->x - 10 - 2, 2 - ht - h - 4, h);
    //			a2b(" %.1f %.1f %.1f %.1f box",
    //				s->x - 10 - 2, 2 - ht - h - 4,
    //				w + 4, h);
    a2b("\n");
