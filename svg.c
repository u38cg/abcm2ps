...
#define D_scclef 9
    {"<use id=\"scclef\" transform=\"translate(0,9.5) scale(0.037)\"\n"
     ...
     D_ucclef},
#define D_pclef 10
    {"<path id=\"pclef\" d=\"m-2.7 9h5.4v-18h-5.4v18\" class=\"stroke\" "
     "stroke-width=\"1.4\"/>\n"},
#define D_hd 11
...
#define D_sh513 41
    {"<g id=\"sh513\">\n"
     "	<path d=\"M-2.5 8.7v-15.4M0 7.8v-15.4M2.5 6.9v-15.4\" "
     "class=\"stroke\"/>\n"
...
#define D_ft513 43
    {"<g id=\"ft513\">\n"
     ...
     "	<path d=\"M1.6 3.5v-13M0 3.5v-13\" class=\"stroke\" "
     "stroke-width=\".6\"/>\n"
     "</g>\n"},
#define D_pshhd 44
    {"<g id=\"pshhd\">\n"
...
#define D_mrep2 59
    {"<g id=\"mrep2\" class=\"fill\">\n"
     ...
     "	<path d=\"M-7 8l14 -10m-14 4l14 -10\" class=\"stroke\" "
     "stroke-width=\"1.8\"/>\n"
     "</g>\n"},
#define D_accent 60
...
#define D_lmrd 62
    {"<g id=\"lmrd\">\n"
     "	<use xlink:href=\"#umrd\"/>\n"
     "	<line x1=\"0\" y1=\"0\" x2=\"0\" y2=\"-8\" class=\"stroke\" "
     "stroke-width=\".6\"/>\n"
     "</g>\n",
     D_umrd},
#define D_grm 63
...
#define D_sld 65
    {"<path id=\"sld\" class=\"fill\" d=\"\n"
     ...
     "	-2.1 5 -5.4 6.8 -7.6 6\"/>\n"},
#define D_emb 66
    {"<path id=\"emb\" d=\"m-2.5 -3h5\" class=\"stroke\" stroke-width=\"1.2\" "
...
#define D_sgno 71
    {"<g id=\"sgno\">\n"
     ...
     "	-4 1.9 -5.9 -1.7 -4.2 -2.6\n"
     "	1.3 -0.7 2.9 1.3 0.7 2\"/>\n"
     "    <line x1=\"-6\" y1=\"-4.2\" x2=\"6.6\" y2=\"-16.8\" "
     "class=\"stroke\"/>\n"
...
#define D_showerror 85
    {"<circle id=\"showerror\" r=\"30\" stroke=\"#ffc0c0\" "
...
#define D_ped 89
    {"<text id=\"ped\" font-family=\"serif\" font-size=\"16\" "
     "font-style=\"italic\"\n"
     "	x=\"-10\" y=\"-4\">Ped</text>\n"},
#define D_pedoff 90
    {"<text id=\"pedoff\" font-family=\"serif\" font-size=\"16\" "
     "font-style=\"italic\"\n"
     "	x=\"-5\" y=\"-4\">*</text>\n"},
};
...
static struct {
  ...
  char *def;
    ...
    {D_brace, "<text id=\"brace\" class=\"music\" x=\"-3\" y=\"0\"\n"
...
static void xml_str_out(char *p) {
  ...
  for (q = p; *p != '\0';) {
    ...
    case '"':
      ...
      break;
    case '&':
      if (*p == '#' || strncmp(p, "lt;", 3) == 0 || strncmp(p, "gt;", 3) == 0 ||
          strncmp(p, "amp;", 4) == 0 || strncmp(p, "apos;", 5) == 0 ||
          strncmp(p, "quot;", 5) == 0)
        continue;
      r = "&amp;";
      break;
...
static void define_head(float w, float h) {
  ...
  static const char svg_font_style[] = ".music {font:24px %s;\n"
                                       "	fill: currentColor}\n";
  static const char svg_font_style_url[] = "@font-face {\n"
                                           "	font-family:music;\n"
                                           "	src:%s}\n"
                                           ".music {font:24px music;\n"
                                           "	fill: currentColor}\n";
  static const char svg_head2[] = "</style>\n"
                                  "<title>";
...
void define_svg_symbols(char *title, int num, float w, float h) {
  char *s;
  unsigned i;
  static const char svg_head3[] = " %s %d</title>\n";
  ...
  if (svg == 2) { /* if XHTML */
    if (file_initialized <= 0) {
      ...
      if (cfmt.bgcolor && cfmt.bgcolor[0] != '\0')
        fprintf(fout, " background-color:%s", cfmt.bgcolor);
      fprintf(fout,
              "}\n"
              "\t@page {margin: 0}\n"
              "\tsvg {display: block}\n"
    ...
    gen_info();
    if (cfmt.bgcolor && cfmt.bgcolor[0] != '\0')
      fprintf(fout, "<rect width=\"100%%\" height=\"100%%\" fill=\"%s\"/>\n",
              cfmt.bgcolor);
  }
...
static void output_font(int span) {
  ...
  if (gcur.font_n[0] == '\0' && (span || !gcur.rgb))
    return;
  ...
  if (gcur.xscale != 1 || gcur.yscale != 1 || gcur.rotate != 0) {
    ...
    if (gcur.xscale != 1 || gcur.yscale != 1) {
      if (gcur.xscale == gcur.yscale)
        ...
        fprintf(fout, "scale(%.3f,%.3f)", gcur.xscale, gcur.yscale);
    }
    if (gcur.rotate != 0) {
      if (gcur.xoffs != 0 || gcur.yoffs != 0) {
        float x, xtmp = gcur.xoffs, y = gcur.yoffs, _sin = gcur.sin,
                 _cos = gcur.cos;
        x = xtmp * _cos - y * _sin;
        y = xtmp * _sin + y * _cos;
...
static void acciac(char *op) {
  ...
  fprintf(fout, "<path d=\"M%.2f %.2fl%.2f %.2f\" class=\"stroke\"/>\n", x, y,
          dx, -dy);
}
...
static void gliss(int squiggle) {
  ...
  a = ar / M_PI * 180;
  len = (x2 - x1 - 14) / cos(ar);
  fprintf(fout, "<g transform=\"translate(%.2f,%.2f) rotate(%.2f)\">\n", x1, y1,
          a);
  if (squiggle) {
    n = (len + 2) / 6;
    x1 = 8;
...
static void stem(char *op) {
  ...
  fprintf(fout, "<path d=\"M%.2f %.2fv%.2f\" class=\"stroke\"/>\n", x, y, -h);
}
...
static void show(char type) {
  ...
  if (span) {
    ...
    fprintf(fout, "<text x=\"%.2f\" y=\"%.2f\"", gcur.xoffs + x,
            gcur.yoffs - y);
    switch (type) {
    case 'c':
...
back:
  ...
  if (type == 'b') {
    ...
    fprintf(fout,
            ...
            gcur.xoffs + gcur.cx - 2, gcur.yoffs - y - gcur.font_s + 2, w + 4,
            gcur.font_s + 1);
  }
  gcur.cx = x + w;
  if (s)
...
stack_dump();
  ...
  case 'a':
    ...
    if (strcmp(op, "arc") == 0 || strcmp(op, "arcn") == 0) {
      ...
      if (gcur.cx != NaN) { // if no newpath
        if (path) {
          path_print("\n\t%c%.2f %.2f",
                     x1 != gcur.cx || y1 != gcur.cy ? 'l' : 'm', x1 - gcur.cx,
                     -(y1 - gcur.cy));
        } else {
          gcur.cx = x1;
          gcur.cy = y1;
      ...
      if (a1 == a2) { /* circle */
        ...
        path_print("\n\ta%.2f %.2f 0 0 %d %.2f %.2f "
                   "%.2f %.2f 0 0 %d %.2f %.2f\n",
                   r, r, op[3] == 'n', x2 - x1, -(y2 - y1), r, r, op[3] == 'n',
                   x1 - x2, -(y1 - y2));
        gcur.cx = x1;
        gcur.cy = y1;
      } else {
  ...
  case 'b':
    ...
    if (strcmp(op, "bitshift") == 0) {
      ...
      shift = pop_free_val();
      if (!stack || stack->type != VAL || shift >= 32 || shift < -32) {
    ...
    if (strcmp(op, "bnum") == 0 || strcmp(op, "bnumb") == 0) {
      setg(1);
      y = gcur.yoffs - pop_free_val();
      x = gcur.xoffs + pop_free_val();
      s = pop_free_str();
      ...
      if (op[4] == 'b') {
        w = 7 * strlen(s);
        fprintf(fout,
                "<rect x=\"%.2f\" y=\"%.2f\" width=\"%.2f\" height=\"12\" "
                "fill=\"white\"/>\n",
                x - w / 2, y - 10, w);
      }
      fprintf(fout,
  ...
  case 'c':
    ...
    if (strcmp(op, "crdc") == 0) {
      ...
      fprintf(fout,
              "<text style=\"font:italic 16px serif\"\n"
  ...
  case 'g':
    ...
    if (strcmp(op, "getinterval") == 0) {
      ...
      if ((unsigned)n >= strlen(s) || (unsigned)count >= strlen(s) - n) {
    ...
    if (strcmp(op, "glisq") == 0 || strcmp(op, "gliss") == 0) {
      gliss(op[4] == 'q');
      return;
    }
    if (strcmp(op, "gt") == 0) {
    ...
    if (strcmp(op, "gu") == 0 || strcmp(op, "gd") == 0) {
      stem(op);
      return;
    }
    if (strcmp(op, "gua") == 0 || strcmp(op, "gda") == 0) {
      acciac(op);
      return;
    }
  ...
  case 'h':
    ...
    if (strcmp(op, "hyph") == 0) {
      ...
      fprintf(fout,
              ...
              "	d=\"M%.2f %.2fh%d\"/>\n",
              d - 5, gcur.xoffs + x, gcur.yoffs - y - gcur.font_s * 0.3,
              d * n + 5);
      return;
    }
    break;
  ...
  case 'm':
    ...
    if (strcmp(op, "mrest") == 0) {
      ...
      fprintf(fout,
              "<use x=\"%.2f\" y=\"%.2f\" xlink:href=\"#mrest\"/>\n"
  ...
  case 'p':
    ...
    if (strcmp(op, "pf") == 0) {
      ...
      fprintf(fout,
              "<text style=\"font:bold italic 16px serif\"\n"
              "	x=\"%.2f\" y=\"%.2f\">%s</text>\n",
              x, y, s + 1);
      free(s);
      return;
...
//fixme: should keep the original string...
  ...
  case 'r':
    ...
    if (strcmp(op, "repbra") == 0) {
      ...
      fprintf(fout, "<text x=\"%.2f\" y=\"%.2f\">", x + 4, y - h);
      xml_str_out(s + 1);
      fprintf(fout,
              "</text>\n"
              "<path class=\"stroke\"\n"
  ...
  case 'S':
    if (strcmp(op, "SL") == 0) {
      ...
      fprintf(fout,
              ...
              m1, m2, a1, -a2, a3, -a4, a5, -a6, -l2, c1, -c2, c3, -c4, c5,
              -c6);
      return;
    }
    if (strcmp(op, "SLW") == 0) {
  ...
  case 's':
    ...
    if (strcmp(op, "search") == 0) {
      ...
      if (p) {
        ...
        e3->u.s = malloc(l3);
        e3->u.s[0] = '(';
        memcpy(&e3->u.s[1], &e2->u.s[l1 + l2 - 2], l3 - 1);
        e3->u.s[l1 + l2 - 1] = '\0';
        push(e3);
        push(e);
        e2->u.s[l1] = '\0';
    ...
    if (strcmp(op, "selectfont") == 0) {
      ...
      if (gcur.font_s != h || strcmp(s, gcur.font_n) != 0) {
        free(gcur.font_n_old);
        gcur.font_n_old = gcur.font_n;
    ...
    if (strcmp(op, "sfs") == 0) {
      ...
      if (h > 0) {
        ...
        y -= h - 1;
        while (--n >= 0) {
          fprintf(fout, "M%.2f %.2fl7 3.2 0 3.2 -7 -3.2z\n", x, y);
          y += 5.4;
        }
      } else {
        x -= 3.5;
    ...
    if (strcmp(op, "sgs") == 0) {
      ...
      y -= h;
      while (--n >= 0) {
        fprintf(fout, "M%.2f %.2fl3 1.5 0 2 -3 -1.5z\n", x, y);
        y += 3;
      }
      fprintf(fout, "\"/>\n");
      return;
    ...
    if (strcmp(op, "su") == 0 || strcmp(op, "sd") == 0) {
      stem(op);
      return;
    }
  ...
  case 't':
    ...
    if (strcmp(op, "tubr") == 0 || strcmp(op, "tubrl") == 0) {
      float dx, dy;
      int h;
  ...
  case 'w':
    ...
    if (strcmp(op, "wln") == 0) {
      ...
      w = pop_free_val();
      fprintf(fout,
              "<path class=\"stroke\" stroke-width=\"0.8\"\n"
              "	d=\"M%.2f %.2fh%.2f\"/>\n",
  ...
  while (--len >= 0) {
    ...
    case ']':
      ...
      for (;;) {
        e2 = stack;
        stack = stack->next;
        if (e2->type == STR && (e2->u.s[0] == '[' || e2->u.s[0] == '{'))
          break;
        e2->next = e->u.e;
        e->u.e = e2;
      }
    ...
    case '%':
      ...
      if (strncmp((char *)q, "A ", 2) == 0) { /* annotation */
        ...
        if (type != 'b' && type != 'e') { /* if not beam */
          sscanf((char *)q + 1, "%d %d %f %f %f %d", &row, &col, &x, &y, &w,
                 &h);
        } else {
          sscanf((char *)q + 1, "%d %d %f %f", &row, &col, &x, &y);
          w = h = 6;
        }
        fprintf(fout,
                "<abc type=\"%c\" row=\"%d\" col=\"%d\" x=\"%.2f\" y=\"%.2f\" "
                "width=\"%.2f\" height=\"%d\"/>\n",
